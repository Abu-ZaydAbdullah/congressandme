...
class ApiTests(unittest.TestCase):
    ...
    def test14(self):
        response = requests.request("GET", url + 'Representatives?q={"filters":[{"name":"state","op":"==","val":"Texas"}]}')
        assert(response.ok)

    def test15(self):
        response = requests.request("GET", url + 'States?q={"filters":[{"name":"abbreviation","op":"==","val":"TX"}]}')
        assert(response.ok)

    def test16(self):
        response = requests.request("GET", url + 'Issues?q={"filters":[{"name":"name","op":"==","val":"Agriculture"}]}')
        assert(response.ok)
